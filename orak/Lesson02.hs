...
infixl 7 `alma` -- infixl == left balról zárójelez, ill a precedenciáját (erősségét műveleti sorban) megadjuk
...
-- egesz2 :: Integer
-- egesz2 = egesz1 ez így lehetetlen, a típusok közt nem engedi a váltást
...
-- különböző típusok fontosak, képet keresd
...
f x = x + 1
-- f 1 működik
-- f (1 :: Integer) nem működik

-- Hogyan csináljunk 4-et a one és a three felhasználásával? (one + three) kéne, csak az nem típushelyes.
-- four = one + three  ez így nem műkszik
four1 = one + fromIntegral three
four2 = fromIntegral one + three
four3 = fromIntegral one + fromIntegral three
-- Mi lesz a típusa a four-nak? :t four
...
0-hoz közelít mindig, tehát pozitív számokon floor, negatív számokon ceiling
...
kerekítés, a legközelebbi páros egész számhoz
-}

----------------------------------------
-- Mintaillesztés    FONTOS
----------------------------------------
...
not' True = False
not' False = True
-- not' _ = False lyuk is működik, csak ha a kódban elöl van akkor minden lehetséges megoldás belemegy és a mögötte lévő esetek redundánsak
-- not' x = _  Nem engedi, csak saját jelzésre használhatjuk
-- konkrét értékes mintaillesztés
