...
numberWords' :: Num n => String -> [(n, String)]
numberWords' str = numberWords'' (words str) where
  numberWords'' :: Num n => n -> [String] -> [(n, String)]
  numberWords'' _ [] = []
  numberWords'' n (w : ws) = (n, w) : numberWords'' (n + 1) ws 
...
intersperse' :: a -> [a] -> [a]
intersperse' _ [] = []
intersperse' a (b : bs) = b : intersperse'' a bs where
  intersperse'' :: a -> [a] -> [ą]
  intersperse'' _ [] = []
  intersperse'' a (b : bs) = a : b : intersperse'' a bs 
...
showList' [] = "[]"
showList' (x : xs) = "[" ++ showlist'' xs where
  showList'' :: Show a => [a] -> String
  showList'' [] = "]"
...
unzip' :: [(a, b)] -> ([a], [b])
unzip' [] = ([], [])
unzip' (ab : abs) = let (a, b) = ab; (as, bs) = unzip' abs in (a : _, b : _)
