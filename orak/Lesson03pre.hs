...
-- egy irányból mehet egy sorrendben, utolsó karakter egy záró (üres) karakter []
-- mivel egy lista, nem lehet közvetlenül az n-edik elemet elérni, hanem mindig végig kell menni rajta
-- 1 : 2 : 3 : []
{-
data [a] = [] | a : [a]
infixr 5 :
...
-- Mi lesz a típusa?
head' :: [a] -> a -- parciális fg, tehát nem tudjuk minden esetre definiálni
-- totális fg: minden esetre definiálva van
head' (elso : _) = elso

-- Definiáld a tail' függvényt MINTAILLESZTÉSSEL, amely egy listának eldobja az első elemét.
-- mindent visszaad csak az elsőt nem
tail' :: [a] -> [a]
tail' (_ : maradek) = maradek
...
-- regexhez hasonló, számrenden elvileg lesz róla szó
-- a : b : ys
-- ys = c : d : efgh...
-- basically nevet adtunk annak a résznek 
...
squares :: [Integer]
squares = [x^2 | x <- [0..5]] -- a nyíl iránya fontos, hoy visszafele mutasson
--         ^^^   ^^^^^^^^^^^
...
onlyUpper = undefined
-- onlyUpper str = [c | c <- str, isUpper c]
...
onlyAs = undefined
-- onlyAs str = [word | word <- words str, word == "a"]
-- words fg, feldarabolja egy listára szavakból
...
firstIsA = undefined
-- firstIsA str = [word | word@('a' : _) <- str]
